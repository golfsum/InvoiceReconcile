...
export function ContactForm() {
  ...
  return (
    <section className="mt-4 border bg-surface p-6 sm:p-8" aria-labelledby="contact-form-heading">
      ...
      <p className="mt-2 text-sm leading-6 text-muted">Support requests go to support@invoicereconcile.com. General enquiries go to contact@invoicereconcile.com. Do not include passwords or unredacted financial files.</p>
      {status.kind === "sent" ? (
        ...
        <form className="mt-6 grid gap-4" action={submit}>
          ...
          <Button className="w-fit" type="submit" size="lg" disabled={status.kind === "sending"}>{status.kind === "sending" ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}{status.kind === "sending" ? "Sending" : "Send message"}</Button>
        </form>
      )}
    </section>
  );
