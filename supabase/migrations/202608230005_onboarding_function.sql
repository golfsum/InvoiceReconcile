...
begin
  ...
  if p_currency_code !~ '^[A-Z]{3}$' then raise exception using errcode = '22023', message = 'Currency is invalid'; end if;
  ...
  insert into public.workspaces (organization_id, name, business_name, accounting_basis, currency_code, timezone, created_by)
  values (created_organization_id, btrim(p_business_name), btrim(p_business_name), p_accounting_basis, upper(p_currency_code), p_timezone, actor_id)
...
revoke all on function public.create_initial_workspace(text, text, text, text, text) from public, anon;
